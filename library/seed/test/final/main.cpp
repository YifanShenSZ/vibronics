...
int main() {
    ...
    at::Tensor init_freq = at::from_blob(data.data(), data.size(), at::TensorOptions().dtype(torch::kFloat64)).clone();
    data = CL::utility::read_vector("final-freq.in");
    at::Tensor final_freq = at::from_blob(data.data(), data.size(), at::TensorOptions().dtype(torch::kFloat64)).clone();
    data = CL::utility::read_vector("transformation-matrix.in");
    at::Tensor T = at::from_blob(data.data(), data.size(), at::TensorOptions().dtype(torch::kFloat64)).clone();
    int64_t dim = sqrt(T.size(0));
    T = T.view({dim, dim});
    T.transpose_(0, 1); // Copied from FORTRAN, so needs a transpose
    data = CL::utility::read_vector("shift-vector.in");
    at::Tensor d = at::from_blob(data.data(), data.size(), at::TensorOptions().dtype(torch::kFloat64)).clone();
